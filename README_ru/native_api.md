...
Примеры и описания основных настроек JivoSDK:
...
>**JWT-токен**. Для подписи **JWT-токена**, потребуется сгенерировать **SECRET** и передать его в Jivo. Хранить **SECRET** только на **backend’e**, иначе безопасность будет нарушена.
...
> [!IMPORTANT]<br>Вызов метода `Jivo.setConfig()` нужно выполнять только в методе `Application.onCreate()` после вызова метода `Jivo.init()`.
