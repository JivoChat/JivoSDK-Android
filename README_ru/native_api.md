...
>**JWT-токен**. Для подписи **JWT-токена**, потребуется сгенерировать **SECRET** и передать его через приложение Jivo (в резделе настроек
>для соответствующего SDK канала). Хранить **SECRET** только на **backend’e**, иначе безопасность будет нарушена.
