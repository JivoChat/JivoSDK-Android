...
![Jivo Logo](./README_ru/jivo_logo.svg)
...
**Внимание!** Если в приложении есть несколько процессов, инициализируйте SDK в главном процессе приложения. 
...
Дополнительный функционал JivoSDK.
---------------------------------

### Оценка качества осблуживания.

При работе с клиентами в чатах часто возникает потребность получения обратной связи для оценки работы сотрудников, а также понимания, что можно улучшить.
Подробная информация и настройки доступны по [ссылке](https://www.jivo.ru/help/customization/kak-vkliuchyt-otsenku-kachestva-obsluzhyvanyia.html)

<img src="./README_ru/jivosdk_rating_ru.png" width=200>
...
Для передачи информации о клиенте требуется вызвать статический метод  `Jivo.setContactInfo()` и передать объект типа `ContactInfo`. Данный объект можно сконфигурировать с помощью класса `Builder`, пример кода ниже:
...
Форма контактов отображается в том случае, если не был вызван статический метод `Jivo.setContactInfo()`, либо небыло передано одно из обязательных полей **email** или **phone**.
...
Для передачи **user-token’a** необходимо вызвать статический метод `Jivo.setUserToken()`. Данный метод рекомендуется вызывать после успешной авторизации пользователя. В параметре которого можно передавать как **JWT-токен**.
...
### Получение информации о разрешении уведомлений.

Для получения информации о разрешении уведомлений нужно использовать статический метод `Jivo.addNotificationPermissionListener()`. Данный метод принимает на вход объект типа `NotificationPermissionListener`, у которого требуется реализовать метод обратного вызова `onNotificationPermissionGranted(isGranted: Boolean)`. В параметр данного метода будет передаваться флаг о наличии разрешения уведомлений. Для корректной работы необходимо реализовать интерфейс `NotificationPermissionListener` в главной `Activity` Данный функийонал работает на Android 13 (API level 33) и выше:
